...
export class Builder {
  ...
  addType (typeName, typeDef) {
    ...
    if ('struct' in typeDef) {
      ...
      const reprTupleFieldSetters = /** @type {{ fieldName: string, fn: string }[]} */([])
      ...
      for (let [fieldName, fieldDef] of Object.entries(typeDef.struct.fields)) {
        const isOptional = representation !== 'map' || fieldDef.optional !== true
        let typeRequired = isOptional
        if (isOptional) {
        ...
        if (representation === 'map') {
          ...
          reqCDec = isOptional ? 'requiredCount--\n            ' : ''
          reprMapFieldCases.push(`        case '${resultFieldName}':
          {
            ${reqCDec}const v = ${nullCase}Reprs${safeReference(fieldTypeName)}(obj[key])
            if (v === undefined) {
              return undefined
            }
            if (v !== value || ret !== obj) {
      ...
      if (representation === 'tuple') {
        ...
        reprTupleFieldSetters.forEach(positionSetter)
...
${typeTupleFieldSetters.map((tfs) => tfs.fn).join('\n')}
    ...
    const ret = []
    let requiredCount = ${typeRequiredMapFieldCount}
    for (let i = 0; i < entries.length; i++) {
      const [key, value] = entries[i]
      switch (key) {
${reprTupleFieldSetters.map((tfs) => tfs.fn).join('\n')}
        default:
          return undefined
      }
    }
...
${implicitSet.join('\n')}${implicitSet.length ? '\n' : ''}    if (requiredCount > 0) {
    ...
    const entries = Object.entries(obj)
    let ret = ${hasRename ? '{}' : 'obj'}
    let requiredCount = ${reprRequiredMapFieldCount}
    for (let i = 0; i < entries.length; i++) {
      const [key, value] = entries[i]
      switch (key) {
...
${reprTransformers.join('\n')}
    ...
    return undefined
      ...
      if ('bytesprefix' in typeDef.union.representation && typeof typeDef.union.representation.bytesprefix === 'object') {
        const typeTransformers = /** @type {string[]} */([])
        const reprTransformers = /** @type {string[]} */([])
        Object.entries(typeDef.union.representation.bytesprefix).forEach(([bytes, innerTypeName]) => {
          if (typeof innerTypeName !== 'string') {
