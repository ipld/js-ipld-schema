  ...
  for (const [typeName, typeDefn] of Object.entries(schema.types)) {
    ...
    if ('struct' in typeDefn) {
      ...
      for (let [fieldName, fieldDefn] of Object.entries(typeDefn.struct.fields)) {
        ...
        // Check for implicit field value and rename
        hasImplicit = false
        let hasRename = false
        let renameValue = null
        ...
        if (isMapRepr && typeDefn.struct.representation && 'map' in typeDefn.struct.representation &&
          ...
          const fieldRepr = typeDefn.struct.representation.map.fields[origFieldName]
          if ('implicit' in fieldRepr) {
            hasImplicit = true
          }
          if ('rename' in fieldRepr) {
            hasRename = true
            renameValue = fieldRepr.rename
          }
        }

        // Use renamed field name for validation if present
        const validationFieldName = hasRename ? renameValue : fieldName
        ...
        const inCheck = !fieldDefn.optional && !hasImplicit ? `'${validationFieldName}' in value &&` : `!('${validationFieldName}' in value) ||`
        if (isMap) {
          const kind = fixTypeScriptType(imports, '@ipld/schema/schema-schema.js#KindMap', false)
          let valueType = getTypeScriptType([], mapValueType)
          valueType = fixTypeScriptType(imports, valueType, false)
          if (mapValueNullable) {
            fieldValidators.push(`      (${inCheck} (${fieldDefn.nullable ? `value.${validationFieldName} === null || ` : ''}(${kind}.is${kind}(value.${validationFieldName}) && Object.values(value.${validationFieldName}).every(v => v === null || ${valueType}.is${valueType}(v)))))`)
          } else {
            fieldValidators.push(`      (${inCheck} (${fieldDefn.nullable ? `value.${validationFieldName} === null || ` : ''}(${kind}.is${kind}(value.${validationFieldName}) && Object.values(value.${validationFieldName}).every(v => ${valueType}.is${valueType}(v)))))`)
          }
          ...
          if (elementNullable) {
            fieldValidators.push(`      (${inCheck} (${fieldDefn.nullable ? `value.${validationFieldName} === null || ` : ''}(Array.isArray(value.${validationFieldName}) && value.${validationFieldName}.every(v => v === null || ${elementType}.is${elementType}(v)))))`)
          } else {
            fieldValidators.push(`      (${inCheck} (${fieldDefn.nullable ? `value.${validationFieldName} === null || ` : ''}(Array.isArray(value.${validationFieldName}) && value.${validationFieldName}.every(${elementType}.is${elementType}))))`)
          }
        } else {
          if (fieldDefn.nullable) {
            fieldValidators.push(`      (${inCheck} (value.${validationFieldName} === null || ${fieldType}.is${fieldType}(value.${validationFieldName})))`)
          } else {
            fieldValidators.push(`      (${inCheck} (${fieldType}.is${fieldType}(value.${validationFieldName})))`)
          }
        }
      }
