...
describe('Generate keyed union types', () => {
  let schema, expectedGo, expectedRust, expectedTypeScript

  before(async () => {
    const contents = await readFile(new URL('./fixtures/gen/unions-keyed.md', import.meta.url), 'utf8')
    const dirEnt = index(parse(contents))
    const schemaText = dirEnt.children.get('test').children.get('schema').hunk.body
    schema = fromDSL(schemaText, { includeComments: true, includeAnnotations: true })
    expectedGo = dirEnt.children.get('test').children.get('golang').hunk.body
...
describe('Generate kinded union types', () => {
  let schema, expectedGo, expectedRust, expectedTypeScript

  before(async () => {
    const contents = await readFile(new URL('./fixtures/gen/unions-kinded.md', import.meta.url), 'utf8')
    const dirEnt = index(parse(contents))
    const schemaText = dirEnt.children.get('test').children.get('schema').hunk.body
    schema = fromDSL(schemaText, { includeComments: true, includeAnnotations: true })
    expectedGo = dirEnt.children.get('test').children.get('golang').hunk.body
...
describe('Generate implicit field values', () => {
  let schema, expectedGo, expectedRust, expectedTypeScript

  before(async () => {
    const contents = await readFile(new URL('./fixtures/gen/implicits.md', import.meta.url), 'utf8')
    const dirEnt = index(parse(contents))
    const schemaText = dirEnt.children.get('test').children.get('schema').hunk.body
    schema = fromDSL(schemaText, { includeComments: true, includeAnnotations: true })
    expectedGo = dirEnt.children.get('test').children.get('golang').hunk.body
...
describe('Generate map representation with field renames', () => {
  let schema, expectedGo, expectedRust, expectedTypeScript

  before(async () => {
    const contents = await readFile(new URL('./fixtures/gen/map-renames.md', import.meta.url), 'utf8')
    const dirEnt = index(parse(contents))
    const schemaText = dirEnt.children.get('test').children.get('schema').hunk.body
    schema = fromDSL(schemaText, { includeComments: true, includeAnnotations: true })
    expectedGo = dirEnt.children.get('test').children.get('golang').hunk.body
...
describe('Generate copy types', () => {
  let schema, expectedGo, expectedRust, expectedTypeScript

  before(async () => {
    const contents = await readFile(new URL('./fixtures/gen/copy-types.md', import.meta.url), 'utf8')
    const dirEnt = index(parse(contents))
    const schemaText = dirEnt.children.get('test').children.get('schema').hunk.body
    schema = fromDSL(schemaText, { includeComments: true, includeAnnotations: true })
    expectedGo = dirEnt.children.get('test').children.get('golang').hunk.body
...
describe('Generate @rustserde annotations', () => {
  let schema, expectedGo, expectedRust, expectedTypeScript

  before(async () => {
    const contents = await readFile(new URL('./fixtures/gen/rustserde.md', import.meta.url), 'utf8')
    const dirEnt = index(parse(contents))
    const schemaText = dirEnt.children.get('test').children.get('schema').hunk.body
    schema = fromDSL(schemaText, { includeComments: true, includeAnnotations: true })
    expectedGo = dirEnt.children.get('test').children.get('golang').hunk.body
